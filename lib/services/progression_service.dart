...
class ProgressionService {
  ...
  // Level 1 -> Level 2 requires 1,000 XP.
  static const int startingXpRequirement = 1000;
  ...
  /// XP required for a local level.
  ///
  /// Level 1 starts at 1,000 XP.
  /// Requirement increases by 8% per level.
  static int baseXpForLevel(int localLevel) {
    if (localLevel <= 1) {
      return startingXpRequirement;
    }

    return (
        startingXpRequirement *
            _pow(1.08, localLevel - 1)
    ).round();
  }
  ...
  /// Adds XP while clearing XP debt first.
  static void addXpWithDebtHandling(
    ...
    if (player.xpDebt > 0) {
      final debtCleared = amount.clamp(
        0,
        player.xpDebt,
      );
